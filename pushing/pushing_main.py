...
def generate_height_map(
    depth_images, camera_poses, intrinsic_depth, map_resolution=0.003
):
    ...
    for i in range(len(depth_images)):
        ...
        transformed_points_homogeneous = (
            transform_matrix @ points_camera_homogeneous.T
        ).T
    ...
    height_map = np.full(
        (len(grid_y), len(grid_x)), fill_value=np.nan, dtype=np.float32
    )
...
def setup_world():
    ...
    terrain_creation_task = TerrainCreation(
        name="TerrainCreation",
        num_envs=num_envs,
...
def scale_rgb_to_depth_fov(rgb_img, intrinsic_rgb, intrinsic_depth):
    ...
    resized_rgb_img = cv2.resize(
        expanded_rgb_img, (depth_W, depth_H), interpolation=cv2.INTER_NEAREST
    )
    # Depth 크롭 영역 저장 (옵션)
...
def process_approach_target(
    ...
    robot_approach(robot, target_position, target_orientation, robot_is_moving,)
...
if __name__ == "__main__":
    ...
    target_orientation_1 = euler_angles_to_quat(
        target_orientation_euler_1, degrees=True, extrinsic=False
    )
    target_orientation_2 = euler_angles_to_quat(
        target_orientation_euler_2, degrees=True, extrinsic=False
    )
    target_orientation_3 = euler_angles_to_quat(
        target_orientation_euler_3, degrees=True, extrinsic=False
    )
    target_orientation_4 = euler_angles_to_quat(
        target_orientation_euler_4, degrees=True, extrinsic=False
    )

    # RGB 카메라 intrinsic 예시
    intrinsic_rgb = {
        "fx": (
            my_robot.rgb_cam.get_focal_length()
            / my_robot.rgb_cam.get_horizontal_aperture()
        )
        * my_robot.rgb_cam.get_resolution()[0],
        "fy": (
            my_robot.rgb_cam.get_focal_length()
            / my_robot.rgb_cam.get_vertical_aperture()
        )
        * my_robot.rgb_cam.get_resolution()[1],
    ...
    intrinsic_depth = {
        "fx": (
            my_robot.depth_cam.get_focal_length()
            / my_robot.depth_cam.get_horizontal_aperture()
        )
        * my_robot.depth_cam.get_resolution()[0],
        "fy": (
            my_robot.depth_cam.get_focal_length()
            / my_robot.depth_cam.get_vertical_aperture()
        )
        * my_robot.depth_cam.get_resolution()[1],
    ...
    while simulation_app.is_running():
        ...
        if world.is_playing():
            ...
            if state == "APPROACH_1":
                # state = process_approach_target(
                #     robot = my_robot,
                #     target_position = first_target_position,
                #     target_orientation = target_orientation_euler_1,
                #     robot_is_moving = robot_is_moving,
                #     robot_just_stopped = robot_just_stopped,
                #     depth_images = depth_images,
                #     camera_poses = camera_poses,
                #     current_state = state,
                #     next_state = "APPROACH_2"
                # )
                robot_approach(
                    my_robot,
                ...
                if robot_just_stopped:
                    rgb, depth = get_camera_image(my_robot, "APPROACH_4")
                    if rgb is not None and depth is not None:
                        camera_pose = get_camera_pose(my_robot)
                        depth_images.append(depth)
                        camera_poses.append(camera_pose)
                ...
                if robot_just_stopped:
                    rgb, depth = get_camera_image(my_robot, "APPROACH_5")
                    if rgb is not None and depth is not None:
                        camera_pose = get_camera_pose(my_robot)
                        depth_images.append(depth)
                        camera_poses.append(camera_pose)
                ...
                if robot_just_stopped:
                    rgb, depth = get_camera_image(my_robot, "APPROACH_6")
                    if rgb is not None and depth is not None:
                        camera_pose = get_camera_pose(my_robot)
                        depth_images.append(depth)
                        camera_poses.append(camera_pose)
            ...
            elif state == "APPROACH_7":
                robot_approach(
                    my_robot,
                    seven_target_position,
                    target_orientation_3,
                    robot_is_moving,
                )
                if robot_just_stopped:
                    rgb, depth = get_camera_image(my_robot, "APPROACH_7")
                    if rgb is not None and depth is not None:
                        camera_pose = get_camera_pose(my_robot)
                        depth_images.append(depth)
                        camera_poses.append(camera_pose)
            ...
            elif state == "APPROACH_8":
                robot_approach(
                    my_robot,
                    eight_target_position,
                    target_orientation_3,
                    robot_is_moving,
                )
                if robot_just_stopped:
                    rgb, depth = get_camera_image(my_robot, "APPROACH_8")
                    if rgb is not None and depth is not None:
                        camera_pose = get_camera_pose(my_robot)
                        depth_images.append(depth)
                        camera_poses.append(camera_pose)
            ...
            elif state == "APPROACH_9":
                robot_approach(
                    my_robot,
                    nineth_target_position,
                    target_orientation_3,
                    robot_is_moving,
                )
                if robot_just_stopped:
                    rgb, depth = get_camera_image(my_robot, "APPROACH_9")
                    if rgb is not None and depth is not None:
                        camera_pose = get_camera_pose(my_robot)
                        depth_images.append(depth)
                        camera_poses.append(camera_pose)
